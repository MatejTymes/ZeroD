...
public class SqlNodeDao implements NodeDao {
    ...
    public SqlNodeDao(DataSource dataSource, Clock clock) {
    ...
    public void registerHeartBeatFor(NodeName nodeName) {
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement selectStatement = connection.prepareStatement("SELECT * FROM node WHERE node_name = ?");
            ...
            if (!exists) {
                PreparedStatement insertStatement = connection.prepareStatement("INSERT INTO node (node_name, last_heart_beat) VALUES (?, ?)");
            ...
            if (exists || insertionFailed) {
                PreparedStatement updateStatement = connection.prepareStatement("UPDATE node SET last_heart_beat = ? WHERE node_name = ?");
    ...
    public Set<NodeName> findNodesAliveSince(ZonedDateTime activeSince) {
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement ps = connection.prepareStatement("SELECT * FROM node WHERE last_heart_beat >= ?");
